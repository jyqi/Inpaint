function rasl_plot(numImage, canonicalImageSize, layout, Fotr, Tr, Rr)
...
for i = 1:xI
    for j = 1:yI
        if yI*(i-1)+j > numImage
            ...
            container ((gap2+1):(end-gap2), (gap2+1):(end-gap2)) = reshape(Fotr(:,yI*(i-1)+j), canonicalImageSize);
...
for i = 1:xI
    for j = 1:yI
        if yI*(i-1)+j > numImage
            ...
            container ((gap2+1):(end-gap2), (gap2+1):(end-gap2)) = reshape(Tr(:,yI*(i-1)+j), canonicalImageSize);
            bigpic{i,j} = container;
...
for i = 1:xI
    for j = 1:yI
        if yI*(i-1)+j > numImage
            ...
            container ((gap2+1):(end-gap2), (gap2+1):(end-gap2)) = reshape(Rr(:,yI*(i-1)+j), canonicalImageSize);
            bigpic{i,j} = container;
...
figure
subplot(1,3,1)
imshow(reshape(sum(Fotr,2), canonicalImageSize),[])
title('average of unaligned D')
subplot(1,3,2)
imshow(reshape(sum(Tr,2), canonicalImageSize),[])
title('average of aligned D')
subplot(1,3,3)
imshow(reshape(sum(Rr,2), canonicalImageSize),[])
title('average of A')