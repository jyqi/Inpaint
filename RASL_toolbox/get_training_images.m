function [fileNames, transformations, numImages, inv_trans] = get_training_images( rootPath, ...
...
elseif strcmp(transformationInit,'SIMILARITY')
    for fileIndex = 1:length(userDirectoryContents),
        ...
        pointFileName = fullfile(pointrootPath, imageName);
...
elseif strcmp(transformationInit,'AFFINE')
    for fileIndex = 1:length(userDirectoryContents)
        ...
        imageFileName = fullfile(rootPath, imageName);
        fileNames{imageIndex} = double(imread(imageFileName));
...
%          S = compute_homography(baseCoords, points(:, 1:3));
        transformations{imageIndex} = S ;
        inv_trans{imageIndex} = ThreePointAffine( points(:,1:3), baseCoords);
    end
else
